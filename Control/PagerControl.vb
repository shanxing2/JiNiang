...
#Region "函数区"
    ...
    Public Sub ShowOperateResultTask(ByVal success As Boolean, ByVal tips As String, Optional ByVal timeout As Integer = 1000)
        Windows2.DrawTipsTask(If(Me.Parent, Me), tips, timeout, success, False)
    End Sub
    ...
    Private Function CanGotoPreviousPage() As Boolean
        If m_CurrentPage = 1 Then
            ShowOperateResultTask(False, "已到第一页")
            Return False
        Else
            Return True
        End If
    ...
    Private Function CanGotoNextPage() As Boolean
        If m_CurrentPage >= TotalPage Then
            ShowOperateResultTask(False, "已到最后一页")
            Return False
        Else
            m_OldPageSize = nudPageSize.Value.ToIntegerOfCulture
            Return True
