...
#Region "字段区"
    Private m_YanTextForm As FrmYanText
    Private m_DanmuColorForm As FrmDanmuColor
...
#Region "常量区"
    ...
    Private Const DanmuDivId = "danmu-msg"
#End Region
...
#End Region
    ...
    Public Sub UpdateChatHistory(ByVal danmu As String)
        Dim p = webDoc.CreateElement("p")
        p.InnerHtml = danmu
        DanmuDiv.AppendChild(p)
        'webChatHistory.Document.Write(danmu)
        'webChatHistory.Document.Write("<br/><br/>")
    ...
    Public Sub ScrollToBottom()
        If webChatHistory.InvokeRequired Then
            Me.BeginInvoke(Sub() webDoc.Window.ScrollTo(0, webDoc.Body.ScrollRectangle.Height))
        Else
            webDoc.Window.ScrollTo(0, webDoc.Body.ScrollRectangle.Height)
        End If
    End Sub
    ...
    Private Sub picYanText_Click(sender As Object, e As EventArgs) Handles picYanText.Click
        If m_YanTextForm Is Nothing Then
            m_YanTextForm = New FrmYanText
        End If

        m_YanTextForm.ShowFollowMousePosition(MouseLeaveAction.Hide, picYanText.Height, True)

        ' 关闭窗体的时候检查，如果用户已经设置了 颜文字 则获取
        If m_YanTextForm.SelectYanText IsNot Nothing Then
            cmbDanmuInput.Text += m_YanTextForm.SelectYanText
        End If
    ...
    Private Sub picDanmuColor_Click(sender As Object, e As EventArgs) Handles picDanmuColor.Click
        If m_DanmuColorForm Is Nothing Then
            m_DanmuColorForm = New FrmDanmuColor(m_User.ViewRoom.Guard.DanmuColorDec)
        End If

        m_DanmuColorForm.ShowFollowMousePosition(MouseLeaveAction.Hide, picDanmuColor.Size.Height, True)
    ...
    Private Sub webChatHistory_DocumentCompleted(sender As Object, e As WebBrowserDocumentCompletedEventArgs) Handles webChatHistory.DocumentCompleted
        webDoc = webChatHistory.Document
        If webDoc Is Nothing Then Return

        SystemMessageSpan = webDoc.GetElementById(SystemMsgSpanId)
        ...
        End If
        DanmuDiv = webDoc.GetElementById(DanmuDivId)
        m_WebScroll = webDoc.GetElementsByTagName("html")(0)
    End Sub
    ...
    Public Sub LiveStatusChanged(ByVal status As LiveStatus)
    ...
    Private Sub webDoc_Click(sender As Object, e As HtmlElementEventArgs) Handles webDoc.Click
    ...
    Private Sub ShowUserNickClickMenu()
        m_SelectViewerId = webDoc.ActiveElement.GetAttribute("data-uid")
        If m_SelectViewerId.IsNullOrEmpty Then Return

        m_SelectViewerNick = webDoc.ActiveElement.GetAttribute("value")
        Dim sql = $"select remark from viewerRemark where userId = '{m_SelectViewerId}'"
        Dim remark = IO2.Database.SQLiteHelper.GetFirst(sql)
        mnitmUseNickAsReMark.Enabled = (remark Is Nothing)
        mnitmReMarkDelete.Enabled = Not mnitmUseNickAsReMark.Enabled
        If remark Is Nothing Then remark = "未设置备注"
        'mnitmNickAndRemark.Text = $"{m_SelectViewerNick}（{remark.ToString}）"
        ' 备注与昵称相同则不显示备注
...
#End Region
    ...
    Private Sub ShowOperateResultTask(ByVal opRst As (Success As Boolean, Message As String, Result As String))
        Windows2.DrawTipsTask(Me, If(opRst.Success, "成功", opRst.Message), 1000, opRst.Success, False)
    End Sub

    Private Sub ShowOperateResultTask(ByVal success As Boolean)
