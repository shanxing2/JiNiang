...
#Region "具体CMD命令解析区"
    Private Sub DANMU_MSG(ByVal userId As String, ByVal upId As String, ByRef jDic As Dictionary(Of String, Object))
        ...
        m_DanmuBuilder.Append(If(userId = CStr(ar2(0)), "[自己]", If(upId = CStr(ar2(0)), "[播主]", $"<input type=""button"" class=""btn-like-span pointer open-menu"" data-uid=""{CStr(ar2(0))}"" value=""{CStr(ar2(1))}"">")))
        Dim timeStamp = ar1(4).ToString
        Dim precision = TimePrecision.Second
        If timeStamp.Length = 13 Then
            precision = TimePrecision.Millisecond
        End If
        m_DanmuBuilder.Append(" ").Append(CLng(timeStamp).ToTimeStampTime(precision).ToString("yyyy-MM-dd HH:mm:ss")).Append("<br/>").Append(jDic("info")(1))
