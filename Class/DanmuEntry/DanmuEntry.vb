...
#Region "函数区"
    ...
    Protected Friend Shared Sub Open()
        ...
        m_CreateDbSql = IO2.Reader.ReadFile(".\res\CreateDb.sql", Encoding.UTF8)

        If m_CreateDbSql.IsNullOrEmpty Then
            Logger.WriteLine("获取建库语句失败")
            Return
        End If
    ...
    Public Shared Async Function GetMedalFromDbAsync(ByVal upUid As Integer) As Task(Of MedalInfo)
        ...
        Using reader = Await IO2.Database.SQLiteHelper.GetDataReaderAsync(sql)
            While Await reader.ReadAsync
                ...
                medal.Detail = MSJsSerializer.Deserialize(Of Concurrent.ConcurrentStack(Of MedalInfo.Datail))(detail)
                Exit While
            End While
        End Using
        ...
        'medal.Detail.Push(New MedalInfo.Datail With {.ViewerUid = 11, .ViewerUnick = "22", .Level = 10})
...
#End If
    ...
    ''' </summary>
    ''' <returns></returns>
    Private Shared Function ParseDanmu(ByVal danmuJson As String) As (Success As Boolean, Message As String)
...
#Region "反序列化获取信息"
                ...
                For Each dm In danmus
                    ...
                    'sb.Append(If(User.Nick = room.nickname, "[自己]", If(User.ViewRoom.UserNick = room.nickname, "[播主]", room.nickname))).Append("  ")
                    sb.Append(If(User.Nick = dm.nickname, "[自己]", If(User.ViewRoom.UserNick = dm.nickname, "[播主]", $"<input type=""button"" class=""btn-like-span pointer open-menu""   data-uid=""{dm.uid}"" value=""{dm.nickname}"">"))).Append(" ")
                    sb.Append(dm.timeline).Append("<br/>").Append(dm.text).Append("<br/><br/>")
                Next
                ' 去掉最后两个 换行
